...
def test_calibration_table_and_metadata():
    cox, test = _fit_cox()
    result = tenure.calibration(cox, test, horizon=90, n_bins=10)
    ...
    assert result.metadata["horizon"] == 90.0
...
def test_well_specified_cox_is_reasonably_calibrated():
    ...
    cox, test = _fit_cox()
    assert tenure.calibration(cox, test, horizon=90).metadata["calibration_error"] < 0.05


def test_plot_calibration_returns_axes():
    cox, test = _fit_cox()
    result = tenure.calibration(cox, test, horizon=90)
